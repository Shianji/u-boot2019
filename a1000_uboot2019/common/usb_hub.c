...
//some hub devices not report bPwrOn2PwrGood,so add delay time
#define HUB_QUERY_DELAY 400
...
static void usb_hub_power_on(struct usb_hub_device *hub)
{
	 ...
	 */
	hub->query_delay = get_timer(0) + max(HUB_QUERY_DELAY, (int)pgood_delay);
	...
	debug("devnum=%d poweron: query_delay=%d connect_timeout=%d\n",
	      dev->devnum, max(HUB_QUERY_DELAY, (int)pgood_delay),
	      max(HUB_QUERY_DELAY, (int)pgood_delay) + 1000);
}
			      ...
			      unsigned short *portstat)
{
	...
	//kingston DataTraveler 3.0 need more time to be stable,bug 23092
	mdelay(200);

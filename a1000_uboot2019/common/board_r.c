...
#ifdef CONFIG_PH_OS_SELF_START
static int initr_ph_os_bootcmd(void)
{
	char bootcmd_ph_os[]="if test ! -e mmc 0:a /uEnv.txt -a ! -e mmc 0:8 /rootfs.ext4.xz; then; if test ! $bootargs_backup = 1; then setenv bootargs $bootargs_backup; fi; ext4load mmc 0:1 0x81ffffc0 Image.itb; bootm 0x81ffffc0#config-fada; else ext4load mmc 0:a 0x85000000 /uEnv.txt; env import -t 0x85000000 $filesize; if test -n $bootcmd_cmd; then run bootcmd_cmd; fi; ext4load mmc 0:2 0x85000000 /uEnv.txt; env import -t 0x85000000 $filesize; setenv boot_rollback 1; run bootcmd_cmd; reset; fi";
	char *bootcmd_bst=env_get("bootcmd");
	env_set("bootcmd_bst",bootcmd_bst);
	env_set("bootcmd",bootcmd_ph_os);
...
static init_fnc_t init_sequence_r[] = {
	...
	initr_mem,
#endif
#if defined(CONFIG_PH_OS_SELF_START)
	initr_ph_os_bootcmd,
#endif
	run_main_loop,
};

...
/**
 * spi_flash_get_board() - specific board data from SPI flash
 *
...
static inline int spi_flash_get(u32 offset, size_t len,
			void *buf, u32 flash_size)
{
	return spi_flash_get_board(offset, len, buf, flash_size);
}
#else
static inline int spi_flash_get(u32 offset, size_t len,
			void *buf, u32 flash_size)

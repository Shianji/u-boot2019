...
static struct usb_endpoint_descriptor *fb_ep_desc(struct usb_gadget *g,
...
int cmd_bst_fastboot_mem(const char *cmdbuf)
{
	/*
	 * fastboot client format: fastboot flash 0x90000000 boot.img
	 * fastboot server format: flash:0x90000000
	 */

	if (!strncmp(cmdbuf, "flash:0x", strlen("flash:0x"))
	    || !strncmp(cmdbuf, "flash:0X", strlen("flash:0X"))) {
		return 1;
	}
...
static void rx_handler_command(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (req->actual < req->length) {
		cmdbuf[req->actual] = '\0';
		if (cmd_bst_fastboot_mem(cmdbuf)) {
			printf("BST fastboot memory download address: %s\n",
			       &(cmdbuf[6]));
			download_addr =
			    (void *)(simple_strtoul(&(cmdbuf[6]), NULL, 0));
			printf("download_addr:0x%p\n", download_addr);
			bst_fastboot_download_mem(download_addr);

 ...
 */
static int do_i2c_read(cmd_tbl_t *cmdtp, int flag,
...
static int do_i2c_write(cmd_tbl_t *cmdtp, int flag,
				int argc, char * const argv[])
{
	uint	chip;
	uint	devaddr, length;
 ...
 */
static int mod_i2c_mem(cmd_tbl_t *cmdtp,
			int incrflag, int flag,
			int argc, char * const argv[])
{
	uint	chip;
	ulong	addr;
	...
	do {
		...
		if (nbytes == 0) {
			...
			if (nbytes) {
				...
				mdelay(CONFIG_SYS_EEPROM_PAGE_WRITE_DELAY_MS);
#endif
				if (incrflag)
					addr += size;
			}
 ...
 */
static int do_i2c_probe(cmd_tbl_t *cmdtp, int flag,
				int argc, char * const argv[])
{
	int j;
	int addr = -1;
 ...
 */
static int do_i2c_loop(cmd_tbl_t *cmdtp, int flag,
				int argc, char * const argv[])
{
	uint	chip;
	int alen;
...
static int do_sdram(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	static const char * const decode_CAS_DDR2[] = {
		" TBD", " 6", " 5", " 4", " 3", " 2", " TBD", " TBD"
	};

	static const char * const decode_CAS_default[] = {
		" TBD", " 7", " 6", " 5", " 4", " 3", " 2", " 1"
	};

	static const char * const decode_CS_WE_default[] = {
		" TBD", " 6", " 5", " 4", " 3", " 2", " 1", " 0"
	};

	static const char * const decode_byte21_default[] = {
		"  TBD (bit 7)\n",
	...
	static const char * const decode_byte22_DDR2[] = {
		"  TBD (bit 7)\n",
		"  TBD (bit 6)\n",
		"  TBD (bit 5)\n",
		"  TBD (bit 4)\n",
	...
	static const char * const decode_row_density_DDR2[] = {
		"512 MiB", "256 MiB", "128 MiB", "16 GiB",
		"8 GiB", "4 GiB", "2 GiB", "1 GiB"
	};

	static const char * const decode_row_density_default[] = {
	...
	if (type != DDR2) {
		printf("Min clock delay,");
		printf(" back-to-back random column addresses %d\n",
			data[15]);
	}
	...
	default:
		...
		if (data[22] & 0x40)
			puts("  TBD (bit 6)\n");
		if (data[22] & 0x20)
			puts("  Upper Vcc tolerance 5%\n");
		else
			puts("  Upper Vcc tolerance 10%\n");
		if (data[22] & 0x10)
			puts("  Lower Vcc tolerance 5%\n");
		else
			puts("  Lower Vcc tolerance 10%\n");
		if (data[22] & 0x08)
			puts("  Supports write1/read burst\n");
		if (data[22] & 0x04)
			puts("  Supports precharge all\n");
		if (data[22] & 0x02)
			puts("  Supports auto precharge\n");
		if (data[22] & 0x01)
			puts("  Supports early RAS# precharge\n");
		break;
	}
	...
	switch (type) {
	case DDR2:
		printf("SDRAM access from clock (2nd highest CAS latency)");
		printf(" 0.%d%d ns\n",
	...
	switch (type) {
	case DDR2:
		printf("SDRAM access from clock (3rd highest CAS latency)");
		printf(" 0.%d%d ns\n",
 ...
 * on error.
 */
static int do_i2c_bus_speed(cmd_tbl_t *cmdtp, int flag,
				int argc, char * const argv[])
{
	int speed, ret = 0;
 ...
 * Returns zero always.
 */
static int do_i2c_reset(cmd_tbl_t *cmdtp, int flag,
				int argc, char * const argv[])
{

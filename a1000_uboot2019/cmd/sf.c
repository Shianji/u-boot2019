...
static int do_spi_flash_read_write(int argc, char * const argv[])
{
	...
	if (strcmp(argv[0], "update") == 0) {
		ret = spi_flash_update(flash, offset, len, buf);
			...
			strncmp(argv[0], "get", 3) == 0 ||
			strncmp(argv[0], "write", 5) == 0) {
		int read;

		read = strncmp(argv[0], "read", 4) == 0;
		if (read) {
			printf("SF: %zu bytes @ %#x %s: ",
				(size_t)len, (u32)offset, "Read");
			ret = spi_flash_read(flash, offset, len, buf);
		}

		read = strncmp(argv[0], "get", 3) == 0;
		if (read) {
			printf("SF: %zu bytes @ %#x %s: ",
				(size_t)len, (u32)offset, "Get");
			ret = spi_flash_get(offset, len, buf, flash->size);
		}
		read = strncmp(argv[0], "write", 5) == 0;
		if (read) {
			printf("SF: %zu bytes @ %#x %s: ",
				(size_t)len, (u32)offset, "Write");
			...
			char * const argv[])
{
	...
	if (strcmp(cmd, "read") == 0 || strcmp(cmd, "get") == 0 ||
		strcmp(cmd, "write") == 0 ||
	    strcmp(cmd, "update") == 0)
...
U_BOOT_CMD(
	...
	"sf get addr offset|partition len	- get `len' bytes starting at\n"
	"				          `offset' or from start of mtd use fast read flash\n"

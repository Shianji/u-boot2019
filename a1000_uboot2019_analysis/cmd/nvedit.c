 ...
 */
static int env_id = 1;//用来标记环境变量是否被修改
...
static int _do_env_set(int flag, int argc, char * const argv[], int env_flag)
{
	...
	if (argc > 1 && argv[1][0] == '-' && argv[1][1] == 'e')
		return do_env_set_efi(NULL, flag, --argc, ++argv);//设置uefi变量
#endif

	while (argc > 1 && **(argv + 1) == '-') {//只能使用-f表示H_FORCE
		char *arg = *++argv;
	...
	/* Delete only ? */
	if (argc < 3 || argv[2] == NULL) {//根据argc和argv判断是否删除变量
...
int env_set(const char *varname, const char *varvalue)
{
	...
	if (!(gd->flags & GD_FLG_ENV_READY))//确保环境变量系统已初始化
		return 1;

	if (varvalue == NULL || varvalue[0] == '\0')
		return _do_env_set(0, 2, (char * const *)argv, H_PROGRAMMATIC);//删除变量
	else
		return _do_env_set(0, 3, (char * const *)argv, H_PROGRAMMATIC);//设置变量
}

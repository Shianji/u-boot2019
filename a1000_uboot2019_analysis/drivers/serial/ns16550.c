...
//#define serial_out(x, y)	out_32(y, x)
//#define serial_in(y)		in_32(y)
#elif defined(CONFIG_SYS_NS16550_MEM32) && (CONFIG_SYS_NS16550_REG_SIZE < 0)
...
#endif
#else
	serial_out(0 , &com_port->mcr);//disable flow ctrl
	serial_out((serial_in(&com_port->mcr)|(0x1<<1)) , &com_port->mcr );//clear rts

	serial_out((serial_in(&com_port->lcr)|(0x1<<7)) , &com_port->lcr); //enable access DLL & DLH
#ifdef RUN_IN_HAPS
	serial_out(40 , &com_port->dll); //set baud rate
	serial_out(0x2c , (char*)com_port+0xc0); //set baud rate
#else
	serial_out(1 , &com_port->dll); //set baud rate
#endif
	serial_out((serial_in(&com_port->lcr)&(~(0x1<<7)) ), &com_port->lcr ); //clear DLAB bit
	 
	serial_out(serial_in(&com_port->lcr)|(0x3), &com_port->lcr); //set data length to 8 bit, 1 stop bit,no parity

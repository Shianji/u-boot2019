...
static int spi_nor_init(struct spi_nor *nor)
{
	...
	if (nor->addr_width == 4 &&
		...
		if (nor->flags & SNOR_F_BROKEN_RESET) {
			printf("enabling reset hack; ");
			printf("may not recover from unexpected reboots\n");
		}
...
/*
 * Write status register 1 byte,set write/erase protect 32M(Byte)
 * for WINBOND or GD25LQ256D
 * Returns negative if error occurred.
 */
static int disable_writeprotect(struct spi_nor *nor)
{
	int ret;
	u8 val;
...
int spi_nor_scan(struct spi_nor *nor)
{
	...
	disable_writeprotect(nor);

#if 0//ndef CONFIG_SPL_BUILD

...
composite_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *ctrl)
{
	...
	u8				endp;
	struct usb_configuration	*c;
#if 0 /* don't need printf */
    printf("%s:bRequestType:0x%02x,bRequest:0x%02x,wValue:0x%04x,wIndex:0x%04x,wLength:0x%04x\n", 
	...
	case USB_REQ_SET_CONFIGURATION:
		...
		/* delay 200ms , make sure configuration is complete*/
		udelay(200000);
		
		/* printf "composite_setup" for tool_chain */
		printf("composite_setup:\n");
		printf("composite_setup: set_config done\n");
		break;

...
static int dwc3_set_inter_clk(struct udevice *child)
{
	const char *usb_mode = NULL;

	usb_mode = fdt_getprop(gd->fdt_blob,
		dev_of_offset(child), "usb_mode", NULL);
	if (!usb_mode) {
		pr_err("usb dr_mode not found\n");
		return 0;
	}
	if (!strcmp(usb_mode, "usb30")) {
		u32 reg;
#if defined(CONFIG_TARGET_BST_A1000B)
		reg = readl(0x33002164);
		reg |= (1<<9);
		writel(reg, 0x33002164);
#else
		reg = readl(0x330020ac);
		reg |= (1<<29);
		writel(reg, 0x330020ac);
#endif
	}
	return 0;
}

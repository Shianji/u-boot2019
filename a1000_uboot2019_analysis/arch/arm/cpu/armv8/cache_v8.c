...
static u64 *create_table(void)
{
	...
	if (gd->arch.tlb_fillptr - gd->arch.tlb_addr > gd->arch.tlb_size)
		panic("Insufficient RAM for page table: 0x%lx > 0x%lx. Please increase the size in get_page_table_size()",
...
static void split_block(u64 *pte, int level)
{
	...
	if (pte_type(pte) != PTE_TYPE_BLOCK)
		panic("PTE %p (%llx) is not a block. Some driver code wants to modify dcache settings for an range not covered in "
		      "mem_map.", pte, old_pte);

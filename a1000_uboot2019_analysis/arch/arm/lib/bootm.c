...
static void boot_jump_linux(bootm_headers_t *images, int flag)
{
	...
	//从images->ep获取内核入口点，kernel_entry的具体地址由linux内核定义，Linux内核镜像文件的第一行代码就是函数kernel_entry
	kernel_entry = (void (*)(void *fdt_addr, void *res0, void *res1,
				void *res2))images->ep;
	debug("## Transferring control to Linux (at address %lx)...\n",
		(ulong) kernel_entry);
	bootstage_mark(BOOTSTAGE_ID_RUN_OS);
	announce_and_cleanup(fake);//打印一些信息并做一些清理工作
	...
	if (!fake) {
		...
		if ((IH_ARCH_DEFAULT == IH_ARCH_ARM64) &&
			...
			armv8_switch_to_el2(0, (u64)gd->bd->bi_arch_number,
					    ...
					    (u64)images->ep,
					    ES_TO_AARCH32);
		}else{//armv8_switch_to_el2是汇编函数，会根据配置传入对应的参数调用armv8_switch_to_el2，然后从该函数进入linux内核
			//kernel_entry((void*)images->ft_addr, 0, 0,0);
			armv8_switch_to_el2((u64)images->ft_addr, 0, 0, 0,
					    images->ep,
					    ES_TO_AARCH64);
			}

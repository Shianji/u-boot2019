...
int hcreate_r(size_t nel, struct hsearch_data *htab)
{
	...
	if (htab->table == NULL)
		return 0;
	//memset((char*)htab->table,0,(htab->size + 1)* sizeof(_ENTRY));
	      ...
	      struct hsearch_data *htab, int flag)
{
	...
	unsigned int idx;
	unsigned int first_deleted = 0;//用于追踪哈希表中第一个被标记为“已删除”（USED_DELETED）的槽位。它的目的是在哈希表的某个位置发生删除操作时，记录下该位置索引，以便后续的操作（如插入新条目）可以复用该位置。
	int ret;

...
int bootm_find_images(int flag, int argc, char * const argv[])
{
	...
	/* 查找initramfs（Initial RAM File System，初始内存文件系统），参数中传入-会跳过加载find ramdisk */
		       ...
		       uint unc_len, ulong *load_end)
{
	...
	*load_end = load;
	 ...
	 */
	 printf("load_buf:%p,  image_buf:%p\n\rimage_len:%lx comp:%d\n\r",
				load_buf, image_buf, image_len, comp);
	switch (comp) {
	case IH_COMP_NONE:
		if (load == image_start)
			break;
	...
	if (!no_overlap && load < blob_end && load_end > blob_start) {
		...
		if (images->legacy_hdr_valid) {
			if (image_get_type(&images->legacy_hdr_os_copy)
					== IH_TYPE_MULTI) {
				puts("WARNING: legacy format ");
				puts("multi component image overwritten\n");
			}
...
{
	boot_os_fn *boot_fn;//指向操作系统启动函数指针
	ulong iflag = 0;//保存中断状态，用于临时禁用中断
	 ...
	 * any error.
	 */
	if (states & BOOTM_STATE_START)//主要用于初始化images结构体,调用boot_start_lmb来初始化内存分配,记录引导过程的开始时间点
		ret = bootm_start(cmdtp, flag, argc, argv);
	//获取内核映像的头部信息，识别内核格式（如 Legacy、FIT、Android）,提取内核映像的相关参数，并存储到images全局结构体中
	...
	/* Load the OS *///将操作系统内核映像解压到目标地址，并确保在加载过程中没有重叠，并刷新缓存确保数据一致性
	...
	/* Relocate the ramdisk *///将ramdisk加载到适当的内存位置
